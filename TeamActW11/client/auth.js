...
export default class Auth {
    ...
    async getCurrentUser(email) {
        try {
            ...
            const user = await makeRequest('users?' + email,'GET',null,this.jwtToken);
            console.log(user[0]);
            return user[0]; //if there more than one user with the same email;
        } catch (error) {
            // if there were any errors display them
            console.log(error);
        }
